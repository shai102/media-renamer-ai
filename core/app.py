...
from tkinter import Listbox, Scrollbar, Toplevel, filedialog, messagebox, simpledialog, ttk
...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler('media_renamer.log', encoding='utf-8'), logging.StreamHandler()],
)
...
class MediaRenamerGUI:
    ...
    def __init__(self, root):
        ...
        self.sf_model = tk.StringVar(value=self.config.get("sf_model", "deepseek-ai/DeepSeek-V3"))
        ...
        self.embedding_model = tk.StringVar(value=self.config.get("embedding_model", "nomic-embed-text"))
        self.prefer_ollama = tk.BooleanVar(value=self.config.get("prefer_ollama", False))
        self.use_embedding_rank = tk.BooleanVar(value=self.config.get("use_embedding_rank", True))
    ...
    def open_settings(self):
        ...
        ttk.Label(f, text="SiliconFlow 模型名:").grid(row=row, column=0, sticky=tk.W, pady=5)
        ttk.Entry(f, textvariable=self.sf_model, width=45).grid(row=row, column=1, pady=5, padx=10)
        row += 1
        ...
        ttk.Label(f, text="Embedding 模型:").grid(row=row, column=0, sticky=tk.W, pady=5)
        ttk.Entry(f, textvariable=self.embedding_model, width=45).grid(row=row, column=1, pady=5, padx=10)
        row += 1
        ...
        ttk.Checkbutton(f, text="启用 Embedding 候选重排 (提升多候选识别率)", variable=self.use_embedding_rank).grid(row=row, column=0, columnspan=2, sticky=tk.W, pady=5)
        row += 1
    ...
    def _can_use_embedding_rank(self):
        ...
        return bool(self.use_embedding_rank.get() and self.ollama_url.get().strip() and self.embedding_model.get().strip())
    ...
    def _rerank_candidates_with_embedding(self, item, query_title, year, is_tv, source_name, candidates):
        ...
        ranked = [c for _, c in scored] + [c for c in candidates if c not in [x[1] for x in scored]]
...
请在下方候选中选择正确条目："""
    ...
    def _select_best_db_match(self, item, query_title, year, is_tv, source_name, candidates):
        ...
        ranked_candidates, emb_pick, emb_msg = self._rerank_candidates_with_embedding(item, query_title, year, is_tv, source_name, candidates)
        if emb_pick:
        ...
        chosen, reason = self._pick_candidate_with_ollama(item, query_title, year, is_tv, source_name, ranked_candidates)
        if chosen:
        ...
        manual_choice = self._request_manual_candidate_choice(item, query_title, source_name, ranked_candidates)
        if manual_choice:
    ...
    def _resolve_db_match(self, item, query_title, year, is_tv, mode, ai_data, g):
        ...
        if merged:
            t_hit, tid_hit, msg_hit, meta_hit = self._select_best_db_match(item, used_query, year, is_tv, source_name, merged)
    ...
    def _bg_update_single_ui(self, idx, title, t_id, msg, meta):
        ...
        try:
            ...
            if is_tv and t_id != "None" and title:
                if mode == "siliconflow_tmdb":
                    ep_n, ep_p, ep_s = fetch_tmdb_episode_meta(t_id, s, e_calc, self.tmdb_api_key.get(), title, self.bgm_api_key.get())
    ...
    def process_task(self, i):
        ...
        try:
                ...
                if self.prefer_ollama.get():
                    if self.ollama_url.get().strip() and self.ollama_model.get().strip():
                        ...
                        if ai_data is None and self.sf_api_key.get().strip():
                            ai_data, ai_msg = fetch_siliconflow_info(pure, self.sf_api_key.get(), self.sf_model.get())
                    else:
            ...
            if is_tv and tid != "None":
                if mode == "siliconflow_tmdb":
                    ep_n, ep_p, ep_s = fetch_tmdb_episode_meta(tid, s, e_calc, self.tmdb_api_key.get(), std_t, self.bgm_api_key.get())
        ...
        try:
            ...
            if is_tv:
                if not is_sub_audio:
                    ...
                    # 保存单集缩略图：优先剧照，其次季海报/剧集海报，保证BGM模式也能产出JPG
