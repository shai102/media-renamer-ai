...
from ui_qt.theme import status_colors, theme_colors
    ...
    def set(self, value: Any) -> None:
        ...
        for trace in list(self._traces):
            try:
                ...
                logging.exception("VarAdapter trace 回调异常")
...
class RootAdapter:
    ...
    def __getattr__(self, name: str):
        # 兜底：未实现的方法转发给 QMainWindow。记 debug 日志便于发现 adapter 缺失的方法，
        # 避免本应由 adapter 提供 tkinter 语义的调用悄悄拿到 Qt 同名实现。
        logging.debug("RootAdapter.__getattr__ 兜底转发到 QMainWindow: %s", name)
...
class TreeAdapter:
    ...
    def item(self, iid: str, key: str | None = None, **kwargs):
        ...
        if "values" in kwargs:
            ...
            if len(vals) >= 4:
                self._apply_status_color(it, vals[3])
        if "open" in kwargs:
    ...
    def _apply_status_color(self, it: QTreeWidgetItem, status_text) -> None:
        """按状态文本给状态列(第5列)着语义前景色，便于一眼区分处理结果。"""
        text = str(status_text or "").strip()
        sc = status_colors(self._dark)
        color = None
        if text:
            if any(k in text for k in ("完成", "成功", "已锁定", "已重命名", "已归档", "已整理", "已刮削", "已写入", "已识别")):
                color = sc["success"]
            elif any(k in text for k in ("待手动", "待确认", "已跳过", "跳过", "请手动")):
                color = sc["warning"]
            elif any(k in text for k in ("失败", "错误", "限流", "超时", "异常", "无结果", "未找到")):
                color = sc["danger"]
            elif any(k in text for k in ("识别中", "处理中", "同步中", "下载", "刮削中", "执行中")):
                color = sc["info"]
        c, _ = theme_colors(self._dark)
        it.setForeground(4, QBrush(QColor(color or c["text"])))
    ...
    def insert(self, parent, index, iid=None, text="", values=None, open=False, tags=()):
        ...
        if values and len(values) >= 4:
            self._apply_status_color(new_item, values[3])
