...
| 桌面交互 | 基于 **PySide6**，支持海报卡片候选选择与深色/浅色主题 |
...
- 桌面界面全面迁移至 **PySide6**，保留原有批量识别 / 手动匹配 / 归档 / 刮削工作流
- 识别链路对齐 Web 版改进：同文件夹多剧集不再串台、中文集数（第108集 / 第二十五话）不再误判为电影
- 过滤 TMDb 候选中的「电影版 / 剧场版」与「之XX篇」分篇误命中，减少凡人修仙传、我推的孩子等误识别
- 修复设置页 Ollama 模型列表卡住、进度条预览后不归零、ComboBox 滚轮误触等 UI 问题
- 打包脚本与主题样式修复：PyInstaller 单文件 EXE 下 Radio / 下拉箭头正常显示

### v3.3 更新

- 对齐桌面端与 Web 端的标题查询保护逻辑，短英文标题、带点号别名不再被候选扩展误伤
- 额外过滤单字母英文碎片查询，减少 `I''s`、缩写名、撇号标题一类作品被脏候选带偏
- 继续收紧候选搜索链路，降低短标题、特殊命名作品进入错误归档或待手动的概率

### v3.2 更新

- 设置窗口现在会记住你手动调整后的宽度和位置，关闭再打开不会恢复到固定尺寸。
- TV / 电影文件名模板后新增 `预览` 按钮，可直接查看当前模板的渲染结果和示例变量。
- 修复保留媒体信息后缀时可能出现的重复 `- -` 分隔符，模板输出更干净。

### v3.1 更新

- 电视剧候选匹配策略收紧：
  - 电视剧不再把资源包年份强制带入 TMDb / BGM 候选匹配，避免多季剧被季包年份误导
  - 电影仍保留年份约束，继续用于同名电影区分
- 模糊候选自动命中逻辑改为保守模式：
  - Embedding 仅负责候选重排，不再单独自动定案
  - 模糊候选优先交给本地 Ollama 或在线 AI 判定
  - AI 无法稳定判定时，直接转为待手动确认，减少自动误归档
- 衍生作品与错误锁定修复：
  - 加强 `前传 / 外传 / reunion / aftershow` 等衍生作品过滤
  - 错误目录中的 `tmdbid` 不再轻易反向锁死后续识别
  - 多季电视剧年份向上回溯时跳过季包目录，减少 `S07/S08` 这类识别串偏
