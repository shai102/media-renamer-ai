...
def fetch_bgm_by_id_raw(subject_id, api_key=""):
    ...
    try:
        response = session.get(
            f"https://api.bgm.tv/v0/subjects/{subject_id}", headers=headers, timeout=15
        )
...
def fetch_bgm_candidates_raw(title, api_key=""):
    ...
    try:
        response = session.get(
            f"https://api.bgm.tv/search/subject/{q}?type=2", headers=headers, timeout=15
        )
        response.raise_for_status()
...
def fetch_tmdb_candidates_raw(title, year=None, is_tv=True, api_key=""):
    ...
    def _request_once(query, year_mode=None):
        ...
        response = session.get(
            f"https://api.themoviedb.org/3/search/{stype}", params=params, timeout=15
        )
        response.raise_for_status()
